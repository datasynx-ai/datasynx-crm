...
describe("calcNpsScore", () => {
  ...
  it("calculates NPS correctly: all promoters → 100", async () => {
    ...
    const responses = [9, 10, 10, 9].map((score) => ({
      surveyId: "s", slug: "a", contactEmail: "a@b.com", score,
      respondedAt: "2026-01-01T00:00:00.000Z", token: "t", sentAt: "2026-01-01T00:00:00.000Z",
    }));
...
describe("savePendingSurvey / recordSurveyResponse", () => {
  it("saves pending and records response by token", async () => {
    vol.fromJSON({});
    const { savePendingSurvey, recordSurveyResponse, loadSurveyResponses } = await import("../../src/core/survey-engine.js");
