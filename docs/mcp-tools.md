# MCP Tools Reference — datasynx-opencrm v2 (79 Tools)

All 79 tools are registered via `server.registerTool()` (MCP SDK v1.x).
...
Get a specific quote or list all quotes for a customer. Status flow:
`draft → sent → viewed → accepted/declined → paid`. Accepted quotes carry the
e-signature receipt (`signature: { name, signedAt, ip }`).

**Input:** `{ quoteNumber?: string, slug?: string }`

### send_quote — Quote-to-Cash (#49)

Mint the public, token-secured quote link. The recipient opens `GET /q/:token`,
sees the rendered quote and can **Accept** (captures name + timestamp + IP as a
signed receipt written next to the quote) or **Decline**. With `STRIPE_API_KEY`
set, a Stripe payment link is attached; the `POST /webhooks/stripe` handler
(signature-verified, replay-protected) flips the quote to `paid`.

Events `quote.accepted` / `quote.declined` / `quote.paid` fire on the outbound
webhook bus (workflow-engine ready), an interaction is logged, and a
Slack/Telegram notification is queued. Tokens are HMAC-signed
(`DXCRM_QUOTE_SECRET`) **and expire**; invalid or expired tokens never reveal
quote data.

**Input:** `{ slug, quoteNumber, validDays? }`

**Returns:** `{ success, quoteNumber, link, paymentLinkUrl?, expiresInDays, status }`

---

## Product & Price Catalog (#50)

A reusable catalog (`.agentic/catalog.json`) so quotes are built from SKUs
instead of re-typing prices. `generate_quote` line items may reference a `sku`
(description/price/tax auto-filled) and/or be free ad-hoc items
(`description` + `unitPrice`). When `vatPercent` isn't given, the catalog's
`taxRate` is used.

### create_product

Create or update a catalog product (upsert by SKU). RBAC: manager+.

**Input:** `{ sku, name, unitPrice, currency?, taxRate?, recurring?: "monthly"|"yearly", description? }`

### list_products

List all catalog products. **Returns:** `{ count, products }`

### update_product

Update fields of an existing product by SKU. RBAC: manager+.

**Input:** `{ sku, name?, unitPrice?, currency?, taxRate?, recurring?, description? }`
...
Declarative if-then rules in `.agentic/workflows.json` react to internal events
(`deal.updated`, `ticket.created`, `quote.accepted`, `quote.paid`,
`email.replied`, `record.created`; wildcards like `deal.*`). When all
conditions match the event payload (dot paths, ops
`eq/neq/gt/gte/lt/lte/contains/exists`), the whitelisted actions run:
`enroll_in_sequence`, `create_ticket`, `create_task`, `log_interaction`,
`update_deal`, `notify` (Slack/Telegram). String args support
`{{payload.path}}` templates.

Safety: every action passes the **autonomy-policy gate** (`auto|approve|block` —
`approve` queues a human approval instead of executing), is written to the
**audit trail** (actor `workflow`), nested events never cascade into a second
workflow pass, and `create_workflow({dryRun:true, sampleEvent})` simulates a
rule against a sample event without saving or executing anything.
...
```json
{
  ...
  "conditions": [{ "field": "deal.stage", "op": "eq", "value": "won" }],
  "actions": [
    { "tool": "enroll_in_sequence", "args": { "slug": "{{slug}}", "contactEmail": "a@acme.com", "sequenceId": "onboarding" } },
...
CLI: `dxcrm workflow add|list|enable|disable`.

---

## Email Engagement Tracking (#45)

Local-first, data-sparing, **default OFF**. The strongest timing signal in sales:
*"the contact is warm, follow up now."* Three tiers, controlled by
`DXCRM_EMAIL_TRACKING=off|reply|opens|clicks|all` (default `off`):

| Tier | What | Public URL? |
|---|---|---|
| `reply` | Correlate inbound replies to prior sends by thread id; record reply latency | No pixel, no URL |
| `opens` | 1×1 signed GIF pixel — `GET /t/o/:token.gif` | Yes (HTTP server) |
| `clicks` | Links rewritten through `GET /t/c/:token` → 302 to the **signed** destination | Yes (HTTP server) |

Stores (both local NDJSON): `.agentic/sent-mail.ndjson` (one record per send) and
`.agentic/email-events.ndjson` (`sent|open|click|reply`). Internal domains
(`DXCRM_INTERNAL_DOMAINS`) are never tracked.

**Security:** tracking tokens are HMAC-signed (`DXCRM_TRACKING_SECRET`) and the
click destination is signed *into* the token — a tampered token gets a 400, never
a redirect (no open-redirect abuse).

### get_email_engagement

Engagement per contact for a customer. Reply tracking works without a pixel.
